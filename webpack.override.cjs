      ...
      new Promise((resolve) => {
        if (watch) {
          ...
          this.on('START', () => console.log('rollup building...'))
            .on('BUNDLE_END', ({ input, duration }) =>
              console.log(`rollup built ${input} in ${duration}ms`),
            )
            .once('END', resolve);
        } else {
...
module.exports = (webpackConfig) => {
  ...
  return {
    ...
    plugins: [
      ...
      new DefinePlugin({
        'process.env.INFURA_KEY': JSON.stringify(
          process.env.INFURA_KEY || '4bf032f2d38a4ed6bb975b80d6340847',
        ),
      }),
